...
class DOMNodeCollection {
  ...
  dup() {
    const copy = this.HTMLels.slice();
    return new DOMNodeCollection(copy);
  }
}
...
const $d = el => {
  if (el instanceof HTMLElement) {
    ...
    return new __WEBPACK_IMPORTED_MODULE_0__dom_node_collection__["a" /* default */]([]);
  }
};
...
$d.create = str => {
  return $d(document.createElement(str));
};
